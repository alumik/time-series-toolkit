...
class TestGenerator(unittest.TestCase):
    ...
    def test_generate_range_index_with_start_and_length_and_freq(self):
        index = tskit.generators.generate_index(start=0, length=5, freq=2)
        self.assertEqual([0, 2, 4, 6, 8], list(index))

    def test_generate_datetime_index_with_start_and_end(self):
        index = tskit.generators.generate_index(start=pd.Timestamp('2000-01-01'), end=pd.Timestamp('2000-01-05'))
    ...
    def test_generate_datetime_index_with_start_and_length_and_freq(self):
        index = tskit.generators.generate_index(start=pd.Timestamp('2000-01-01'), length=5, freq='5D')
